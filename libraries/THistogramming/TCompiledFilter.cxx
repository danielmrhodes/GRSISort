...
void TCompiledFilter::Load(std::string name) {
  TCompiledFilter other(name);
  swap_lib(other);
}
...
bool TCompiledFilter::MatchesCondition(std::shared_ptr<TUnpackedEvent> detectors) {
  ...
  obj.SetDetectors(std::move(detectors));
  return func(obj);
}
